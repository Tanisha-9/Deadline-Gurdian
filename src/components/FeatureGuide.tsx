...
export default function FeatureGuide({
  ...
  const getTooltipStyle = () => {
    ...
    if (spaceBelow >= 240 || spaceBelow > spaceAbove) {
      ...
      top = rect.top - 210; // offset tooltip estimated height
      if (top < 16) top = 16;
    }
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 overflow-hidden">
        ...
        <motion.div
          key={step}
          initial={{ opacity: 0, scale: 0.95, y: step === 1 ? 15 : 0 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.95 }}
          transition={{ duration: 0.2 }}
          style={tooltipStyle}
          className={`bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-5 md:p-6 overflow-hidden flex flex-col`}
        >
          ...
          {/* Persona selector explicitly nested inside Step 1 */}
          {step === 1 ? (
            <div className="space-y-5">
              <div className="space-y-1.5">
                <div className="flex items-center gap-2">
                  <Sparkles className="h-5 w-5 text-yellow-400 animate-pulse" />
                  <span className="text-xs font-mono uppercase tracking-wider text-slate-500">Workspace Onboarding</span>
                </div>
                <h3 className="text-xl font-bold text-white tracking-tight">Select Productivity Persona</h3>
              ...
              <div className="grid grid-cols-1 gap-2.5">
                ...
                >
                  <GraduationCap className={`h-6 w-6 shrink-0 ${persona === 'student' ? 'text-emerald-400' : 'text-slate-500'}`} />
                  <div>
                ...
                >
                  <Briefcase className={`h-6 w-6 shrink-0 ${persona === 'professional' ? 'text-indigo-400' : 'text-slate-500'}`} />
                  <div>
                ...
                >
                  <TrendingUp className={`h-6 w-6 shrink-0 ${persona === 'entrepreneur' ? 'text-amber-400' : 'text-slate-500'}`} />
                  <div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-800/80 mt-5">
            ...
            >
              {step === 1 ? 'Start Tour' : step === steps.length ? 'Done' : 'Next'}
